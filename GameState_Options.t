...
procedure GameState_Options_Update()
	...
	if options_select = 4 then
		...
		if opt_scrollspeed < 0.1 then
			opt_scrollspeed := 0.1
		end if
		if opt_scrollspeed > 10.0 then
			opt_scrollspeed := 10.0
		end if
		Font.Draw("SCROLL SPEED: " + realstr(opt_scrollspeed, 0), 30, maxy - 90 - (4 * 24)- floor(options_y), font_regular, 52)
	else
		Font.Draw("SCROLL SPEED: " + realstr(opt_scrollspeed, 0), 30, maxy - 90 - (4 * 24)- floor(options_y), font_regular, 54)
	end if
